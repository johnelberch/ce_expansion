...
from ce_expansion.atomgraph.bcm_prime import BCModelAds
...
# NOTE: This function seems to work by defining a vector array with the ordering. In other words, ordering is defined as a 1D vector of length equal to Natoms. Values represent the atom types
# Thus, I believe it needs no change (for now)
@functools.total_ordering
class NanoparticleAds:
    def __init__(self, bcm: BCModelAds, composition: Iterable[int],
                 ordering: Iterable[int] = None):
        """
        Nanoparticle object for GA simulations (patched to accept adsorbates)
        Represents a single structure with a given chemical ordering (arr)

        Args:
            bcm: instantiated BCModelAds to compute score as function of ordering
            composition: array of metal compositions (must be metal amounts as integers, sorted by alphabetical order):
                        [# metal1, # metal2, ..., # metaln]

        KArgs:
            ordering: array representing positions in nanoparticle and whether they're
                    occupied by metal1 (0), metal2 (1), ..., or metaln (n-1)
    ...
    def __lt__(self, nanop2: NanoparticleAds) -> bool:
        """NP comparisons (< > == !=) use self.ce"""
        return self.ce < nanop2.ce

    def __eq__(self, nanop2: NanoparticleAds) -> bool:
        """NP comparisons (< > == !=) use self.ce"""
        return self.ce == nanop2.ce

    def copy(self) -> NanoparticleAds:
        """
        ...
        """
        return NanoparticleAds(self.bcm, self.composition, ordering=self.ordering.copy())

    def mate(self, nanop2: NanoparticleAds) -> List[NanoparticleAds]:
        """
        ...
        if (child1 == child2).all():
            children = [NanoparticleAds(self.bcm, self.composition, c)
        ...
        # return the children
        return [NanoparticleAds(self.bcm, self.composition, c)
                for c in [child1, child2]]
...
class GA(object):
    def __init__(self, bcm: BCModelAds,
        ...
        Args:
        bcm: BCModelAds (initiated so it contains the proper mapping and atom properties)
    ...
    def __getitem__(self, i: int) -> NanoparticleAds:
        return self.pop[i]
    ...
    def run(self, max_gens: int = -1, max_nochange: int = 750,
        ...
        if not self.random and self.use_metropolis:
            ...
            if opt_ce < best.ce:
                ...
                new_best = NanoparticleAds(self.bcm, self.composition, opt_order)
    ...
    def _initialize_pop(self):
        ...
        # plus current best found in database (if available)
            ...
            spikes = [NanoparticleAds(self.bcm, self.composition, cn_case)
            ...
            if self.prev_results:
                nanop = NanoparticleAds(self.bcm, self.composition, self.prev_results.ordering)
                self.pop.append(nanop)

        # create random structures for remaining popsize
        nanops_needed = self.popsize - len(self.pop)
        self.pop.extend(NanoparticleAds(self.bcm, self.composition)
...
def build_ga(atoms: ase.Atoms, gamma_values: Dict[str, Dict[str,float]],
