...
from ase.data import colors, chemical_symbols
...
from ce_expansion.ga.ga_prime import GA
...
def make_bcm(atoms: ase.Atoms, gamma_values: Dict[str, Dict[str,float]], mapping: Optional[Dict[str,str]] = {}, bond_list=None, CN_Method='frac') -> BCModelAds:
    ...
    Returns:
        bcm (BCModelAds): BCModelAds object (BCM for adsorbed cases)
    """

    return BCModelAds(atoms=atoms, gamma_values=gamma_values, bond_list=bond_list, CN_Method=CN_Method, mapping=mapping)
...
class Nanoparticle:
    def __init__(self, atoms: ase.Atoms, gamma_values: Dict[str, Dict[str,float]], mapping: Optional[Dict[str,str]] = {}, bond_list: Optional[np.ndarray] = None, 
                 x: float = 1.2, describe: str = "none", method: str = "frac", spike: bool = False, cut_coord: str = "x"):
        """Initialize the Nanoparticle object.  This is a wrapper for the BCModel object and the GA object.  
        The BCModel object is helpful for calculating the CE of the atoms object as well as to calculate the coordination numbers of the atoms object and finding the shell numbers.  
        The GA object is helpful for finding the optimal chemical ordering of the atoms object using the BCModel.
        ...
        Kwargs:
            mapping (dict): Mapping dictionary with (key,value) pairs corresponding to (fake element, original element). e.g., {'Np' : 'Au'} for 'Np' atoms representing 'Au' atoms with adsorbates.
            bond_list (np.ndarray) : Matrix of dimensions (-1, 2) containing information on bonds. With bonding atom i index in first column, and target atom j in second column.
            ...
            cut_coord (str, optional): Cutting coordinate for cut plot.
        """
        ...
        # Composition
        self.unique_metals = sorted(set(self.atoms.symbols))
    ...
    def core_shell_plot(self,save=False,saveas='NP_Comp',dpi=300):
        ...
        for i, key in enumerate(list(self.comps.keys())):
            ...
            element_color = self.dict_colors[key]
